...
// カード個別静的ページ(cards/<id>/、build-card-pages.mjsが生成)への基点パス(常に相対パス)。
// アプリ側からこのページへ通常のa[href]リンクを張る際に使う。
const CARD_PAGE_BASE_PATH = 'cards/';

// ---- Supabase Auth連携設定(Stage 1B) ----
...
const DEFAULT_REGULATIONS = [
  ...
  {
    id: 'normal-school', name: 'ノーマルスクール', builtin: true,
    minMain: 40, maxMain: null, maxCopies: 4, sideMax: 10, totalMax: 60,
    // 使用可能なレアリティの許可リスト(この配列に無いレアリティのカードは1枚でも含まれるとNG)。
    allowedRarities: ['N'],
    // 禁止カードはカード名ではなくcardId配列で管理する(色違い等でも表記ゆれなく確実に特定できるため)。
    // 追加する場合は既存カードIDの形式("1-23"等)でここに追記する。
    // 【禁止理由】いずれもレアリティNのカードで、allowedRaritiesの判定だけでは弾けないため個別に指定する。
    // (「オブシディアン」はカードデータ上のレアリティがR=レアであり、allowedRarities:['N']の判定だけで
    // 既に使用不可になるため、ここには含めない。初期版でN表記だった誤植を理由に禁止指定されたものと
    // 思われるが、現在のカードデータ(レアリティR)を正として扱う。)
    bannedCardIds: [
      '1-48', // 円形闘技場
      '2-36', // 足利義政
      '2-9',  // 石田三成
    ...
    note: 'Nのみ使用可能。一部禁止カードあり。同名カードは4枚まで（「デッキに何枚でも入れてよい」カードを除く）。メイン40枚以上。',
  },
  {
    id: 'bo1', name: 'BO1', builtin: true,
    // メインはゲームルール上の上限なし(maxMain: null)。サイドは0枚固定。
    // アプリ内部のDoS対策上限(DECK_ENTRIES_MAX・DECK_QTY_MAX。05-deck-logic.js側の
    // sanitizeCardEntries/deckAddCardで一律に適用)は、レギュレーションに関わらず
    // 常に効いているため、ここでは何も指定しない。
    minMain: 40, maxMain: null, maxCopies: 4, sideMax: 0,
    note: 'サイドなし。メイン40枚以上、上限なし。',
  },
];
...
// ---- デッキ全体のDoS対策としての内部安全上限(ゲームルール上の上限とは別物) ----
// 以前の実地検証で、sanitizeCardEntriesには1エントリあたりのqty上限(DECK_QTY_MAX)はあっても
// エントリ「件数」自体の上限が無く、異なるcardIdを持つエントリを数十万件含むペイロードが
// そのまま通過することを確認した(実測: 50万件の異なるcardIdがそのまま通過)。BO1のように
// 「ゲーム上のメイン枚数上限なし」を許容するレギュレーションを導入する以上、メイン/サイド
// それぞれについて、現実のプレイでは絶対に到達しない余裕を持った「件数」の上限を設ける。
//   RAW_ENTRY_LIST_MAX: sanitizeCardEntriesが受け取る「マージ前」の生配列の要素数上限。
//     マージ処理(Map構築)そのものに時間がかかることを防ぐための、最初の安価なチェック。
//   DECK_ENTRIES_MAX:   マージ後の「異なるcardIdの件数」上限(メイン/サイド各ゾーンごと)。
// 超過した場合は一部だけ残す(切り詰める)のではなく、そのゾーンのリスト全体を無効(空)にする。
// 数十万件規模のデータから「どの何百件を残すか」に合理的な基準がなく、部分的に残すと
// 見かけ上は正常に読み込めたかのように見えてしまうため。
// 【合計枚数(qty合計)には上限を設けない】これは意図的な設計判断であり、以下の理由による。
// 通常操作(deckAddCard)や外部データの新規取り込み(sanitizeCardEntries)では、1エントリあたりの
// qtyはDECK_QTY_MAX(999)で上限される。ただし、サイド上限0のレギュレーションへ変更した際に
// sideCardsをmainCardsへ合算するmergeSideIntoMainIfNoSide(05-deck-logic.js)は、既存の
// 正当な値を壊さない(サイドカードを黙って削除しない)ことを優先し、合算後の1エントリが
// DECK_QTY_MAXを超えることを意図的に許容している。そのため「1ゾーンの合計枚数は自動的に
// DECK_ENTRIES_MAX×DECK_QTY_MAX以下に収まる」とは言えない(この関数は現在mainCards/sideCardsの
// 2ゾーンの合算にしか使っていないが、将来3つ以上のソースを合算するように拡張された場合でも
// 成立する説明として、特定のソース数を前提にした上限値は書かない)。
// 合算後のqty自体はDECK_QTY_MAXを超え得るが、あくまで有限の整数値(JavaScriptのNumberとして
// 扱える範囲内)であり、件数(エントリの個数)にはDECK_ENTRIES_MAXが変わらず適用されている。
// 処理コストやメモリ使用量は、qtyという単なる数値フィールドの大小ではなく、エントリの件数
// (Map/配列の要素数)に支配されるため、合計枚数(qty合計)に追加の上限を設けなくても影響は
// 無視できる規模にとどまる。既存のdecompression-bomb.test.jsには、圧縮後サイズの上限確認の
// ために300件×qty999(合計約30万)のデッキを正しく往復できることを検証する回帰テストもあり、
// 合計枚数に上限を設けるとこの既存の正常系も壊れるため、合計枚数の上限は設けない。
const RAW_ENTRY_LIST_MAX = 2000;
const DECK_ENTRIES_MAX = 500;
